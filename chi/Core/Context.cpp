...
#include "chi/CodeSupport/LLVMVersion.hpp"

#include "chi/Runtime/Runtime.hpp"
...
std::vector<std::string> Context::listModulesInWorkspace() const noexcept {
	...
	for (const auto& dirEntry : boost::make_iterator_range(
		...
		if ((fs::is_regular_file(p) || fs::is_symlink(p)) && p.extension() == ".chimod") {
...
Result Context::loadModule(const fs::path& name, ChiModule** toFill) {
	...
	if (!fs::is_regular_file(fullPath) && !fs::is_symlink(fullPath)) {
...
// link it in
		...
		if (mod.shortName() == "main") {
			...
			res += chigraphRuntime(llvmContext(), &runtimeMod);
			if (!res) { return res; }
...
fs::path workspaceFromChildPath(boost::filesystem::path path) {

	// initialize workspace directory
	// go up until it is a workspace
	while (!path.empty() && !fs::is_regular_file(path / ".chigraphworkspace") && !fs::is_symlink(path / ".chigraphworkspace")) {
		path = path.parent_path();
	}

	return path;  // it's ok if it's empty
}
