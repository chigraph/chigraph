...
// clang 3.7- don't do this right at all. We need it to pass our arg0, and not just "clang" like 3.7
// does.
...
std::unique_ptr<CompilerInvocation> createInvocationFromCommandLineBackport(
	...
	if (Jobs.size() > 1) {
		for (auto& A : C->getActions()) {
			...
			if (isa<driver::BindArchAction>(A)) A = *A->begin();
		}
	}
	if (Jobs.size() == 0 || !isa<driver::Command>(*Jobs.begin()) || (Jobs.size() > 1)) {
		SmallString<256>          Msg;
	...
	if (!CompilerInvocation::CreateFromArgs(*CI, const_cast<const char**>(CCArgs.data()),
...
std::unique_ptr<llvm::Module> cToLLVM(LLVMContext& ctx, const char* execPath, const char* code,
	...
	if (strcmp(code, "") != 0) { compileArgs.push_back(fileName); }
...
int main(int argc, char** argv) {
	...
	general.add_options()("help,h", "Produce Help Message")(
	    "clangargs,c", po::value<std::vector<std::string>>()->default_value({}, ""),
	    "Extra arguments to pass to clang")("fakename,f",
	                                        po::value<std::string>()->default_value("internal.c"));
	...
	auto              mod =
	    cToLLVM(ctx, argv[0], code.c_str(), vm["fakename"].as<std::string>().c_str(), strArgs);
