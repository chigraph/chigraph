...
NodeInstance::NodeInstance(std::unique_ptr<NodeType> nodeType, float arg_x, float arg_y, std::string id_)  
	...
	NodeInstance& lhs, size_t connectionInputID, NodeInstance& rhs, size_t connectionOutputID)
{
	...
	if (connectionInputID >= lhs.outputDataConnections.size()) {
		...
		for(auto& output : rhs.type->dataOutputs) {
			dataOutputs.push_back({{output.second, lhs.type->context->stringifyType(output.first)}});
		}
