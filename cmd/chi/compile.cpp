...
int compile(const std::vector<std::string>& opts) {
	...
	if (!fs::is_regular_file(infileRelToPwd) && !fs::is_symlink(infileRelToPwd)) {
		auto infileRelToSrc = fs::absolute(infile, c.workspacePath() / "src");

		// if we still didn't find it, then error
		if (!fs::is_regular_file(infileRelToSrc) && !fs::is_symlink(infileRelToPwd)) {
	...
	// get module name
	// fs::relative follows symlinks, which is undesirable
	auto moduleName = infile.lexically_relative(c.workspacePath() / "src").replace_extension("");

	std::cerr << "workspacepath: " << c.workspacePath().string() << " Modname: " << moduleName.string() << " infile: " << infile << std::endl;
