...
Result ModuleCache::cacheModule(const boost::filesystem::path& moduleName, llvm::Module& compiledModule, std::time_t timeAtFileRead) {
...
boost::filesystem::path ModuleCache::cachePathForModule(const boost::filesystem::path& moduleName) const {
...
std::unique_ptr<llvm::Module> ModuleCache::retrieveFromCache(const boost::filesystem::path& moduleName, std::time_t atLeastThisNew) {
	...
	if (cacheEditTime < atLeastThisNew) {
		return nullptr;
	}
	...
	auto errorOrMod = llvm::parseBitcodeFile(llvm::MemoryBufferRef(*bcFileBufferOrError.get()), context().llvmContext());
