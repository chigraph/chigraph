...
// The linux implementation finds gcc and runs what's basically gcc -xc -E -v - < /dev/null which prints out the search paths.
...
Result stdCIncludePaths(std::vector<boost::filesystem::path>& toFill) {
	...
	if (envIncPath != nullptr) {
		...
		std::transform(parsedOut.begin(), parsedOut.end(), std::back_inserter(toFill), [] (auto p) { return fs::path{p}; });
		return res;
	}
	...
	auto iter = std::find(stdErrLines.begin(), stdErrLines.end(), "#include <...> search starts here:");
	if (iter == stdErrLines.end()) {
		res.addEntry("EUKN", "Failed to find \"#include <...> search starts here:\" in gcc stderr", {{"Stderr", stdErrData}});
		return res;
	}
	...
	for (; iter != stdErrLines.end() && *iter != "End of search list."; ++iter) {
		...
		// append it to `toFill`
		assert(fs::is_directory(incPath));
