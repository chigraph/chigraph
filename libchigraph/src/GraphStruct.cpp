...
std::vector<NodeInstance*> GraphStruct::setName(std::string newName, bool updateReferences) {
	...
	if (updateReferences) {
		...
		auto makeInstances = context().findInstancesOfType(module().fullNamePath(), "_make_" + oldName);
		...
		auto breakInstances = context().findInstancesOfType(module().fullNamePath(), "_break_" + oldName);
...
void GraphStruct::modifyType(size_t id, DataType newTy, std::string newName, bool updateReferences) {
...
void GraphStruct::updateNodeReferences() {
	...
	auto breakInstances = context().findInstancesOfType(module().fullNamePath(), "_break_" + name());
